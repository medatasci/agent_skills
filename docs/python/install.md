...
Codex skill install, remove, list, download, and install path resolution module.
...
`list` and path-resolution helpers are read-only.
...
Filesystem writes:
Install, download, and remove write or delete local files.
