# Encephalomalacia Research Plan

Status: not_started

## Scope Definition

- Disease or finding: Encephalomalacia
- MR-RATE original name: Encephalomalacia
- Ontology source: snomed
- SNOMED CT: 58762006 (Encephalomalacia)
...
1. Describe how Encephalomalacia is expected to present on diagnostic imaging, especially MRI, using radiology-report style language.
2. Identify findings that support or argue against Encephalomalacia relative to common mimics and relevant MR-RATE categories.
...
```text
As a radiology expert, given a diagnosis of Encephalomalacia, what would you look for in MRI images to confirm?
```

Advanced prompt:

```text
As a neuroradiologist, given a diagnosis of Encephalomalacia, what sequence-specific MRI signal characteristics, lesion morphology, anatomic distribution, chronicity features, volume-loss patterns, and associated findings would you expect to see on MRI?
```

Search variants:

- Encephalomalacia MRI imaging characteristics
- Encephalomalacia MRI T1 T2 FLAIR DWI ADC SWI contrast signal characteristics
- Encephalomalacia MRI lesion morphology anatomic distribution
- Encephalomalacia MRI chronicity features structural patterns
- Encephalomalacia MRI associated findings radiology
- Encephalomalacia MRI differential diagnosis mimics
- Encephalomalacia MRI report language findings impression
- Encephalomalacia clinical course progression stable treatment response
- Encephalomalacia cohort definition endpoint misclassification adjudication
