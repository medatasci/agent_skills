# Mastoiditis Research Plan

Status: not_started

## Scope Definition

- Disease or finding: Mastoiditis
- MR-RATE original name: Mastoiditis
- Ontology source: snomed
- SNOMED CT: 52404001 (Mastoiditis)
...
1. Describe how Mastoiditis is expected to present on diagnostic imaging, especially MRI, using radiology-report style language.
2. Identify findings that support or argue against Mastoiditis relative to common mimics and relevant MR-RATE categories.
...
```text
As a radiology expert, given a diagnosis of Mastoiditis, what would you look for in MRI images to confirm?
```

Advanced prompt:

```text
As a neuroradiologist, given a diagnosis of Mastoiditis, what sequence-specific MRI signal characteristics, lesion morphology, anatomic distribution, chronicity features, volume-loss patterns, and associated findings would you expect to see on MRI?
```

Search variants:

- Mastoiditis MRI imaging characteristics
- Mastoiditis MRI T1 T2 FLAIR DWI ADC SWI contrast signal characteristics
- Mastoiditis MRI lesion morphology anatomic distribution
- Mastoiditis MRI chronicity features structural patterns
- Mastoiditis MRI associated findings radiology
- Mastoiditis MRI differential diagnosis mimics
- Mastoiditis MRI report language findings impression
- Mastoiditis clinical course progression stable treatment response
- Mastoiditis cohort definition endpoint misclassification adjudication
