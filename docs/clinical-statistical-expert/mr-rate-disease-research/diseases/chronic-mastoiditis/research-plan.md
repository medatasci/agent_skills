# Chronic mastoiditis Research Plan

Status: not_started

## Scope Definition

- Disease or finding: Chronic mastoiditis
- MR-RATE original name: Chronic mastoiditis
- Ontology source: snomed
- SNOMED CT: 80645004 (Chronic mastoiditis)
...
1. Describe how Chronic mastoiditis is expected to present on diagnostic imaging, especially MRI, using radiology-report style language.
2. Identify findings that support or argue against Chronic mastoiditis relative to common mimics and relevant MR-RATE categories.
...
```text
As a radiology expert, given a diagnosis of Chronic mastoiditis, what would you look for in MRI images to confirm?
```

Advanced prompt:

```text
As a neuroradiologist, given a diagnosis of Chronic mastoiditis, what sequence-specific MRI signal characteristics, lesion morphology, anatomic distribution, chronicity features, volume-loss patterns, and associated findings would you expect to see on MRI?
```

Search variants:

- Chronic mastoiditis MRI imaging characteristics
- Chronic mastoiditis MRI T1 T2 FLAIR DWI ADC SWI contrast signal characteristics
- Chronic mastoiditis MRI lesion morphology anatomic distribution
- Chronic mastoiditis MRI chronicity features structural patterns
- Chronic mastoiditis MRI associated findings radiology
- Chronic mastoiditis MRI differential diagnosis mimics
- Chronic mastoiditis MRI report language findings impression
- Chronic mastoiditis clinical course progression stable treatment response
- Chronic mastoiditis cohort definition endpoint misclassification adjudication
