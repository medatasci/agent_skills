#!/usr/bin/env python
"""Audit Codex Desktop rollout history for PowerShell shell_command problems."""
...
SOLUTION_PATTERN_DEFS = {
    "shell-dialect-mismatch": {
        ...
        "canonical_example": "@'\nfrom pathlib import Path\nprint(Path.cwd())\n'@ | & $py -",
        "target_skill": "powershell-caller",
    },
    ...
    "unmapped-diagnostic-discovery": {
        ...
        "target_skill": "powershell-failure-analyst",
    },
    ...
    with sqlite3.connect(sqlite_path) as conn:
        ...
        conn.executescript(
            ...
            create view v_event_classes as
              select run_id, error_family, error_class, severity_label,
              ...
              from events
              group by run_id, error_family, error_class, severity_label;
...
def write_outputs(args: argparse.Namespace, summary: dict, events: list[dict]) -> dict:
    ...
    prefix = args.prefix or f"powershell-failures-{args.mode}-{start}_to_{end}"
    ...
    with open(md_path, "w", encoding="utf-8") as handle:
        handle.write("# PowerShell Failure Analysis\n\n")
