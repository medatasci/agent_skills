...
- `events`: one row per detected diagnostic, labeled by `detector_id`, `tool_name`, `tool_family`, `runtime_environment`, `diagnostic_language`, `error_family`, `error_class`, severity, confidence, rework, and repair mode.
- `runs`: one row per analyzer run with shell-call counts, detected-event counts, efficiency percentages, lost-time estimate, and formulas.
- `efficiency_by_error_family`: lost-time and repair-turn estimates grouped by `error_family`.
...
- `v_efficiency_summary`, `v_lost_time_by_error_family`, `v_event_classes`, `v_rule_priority`, `v_pattern_priority`, `v_repair_modes`: query-ready views.

## Category Labeling Process

Use layered labels so rollups stay stable:

1. Extract evidence signatures from output: detector, tool name, runtime environment, exit code, structured diagnostic IDs, parser text, command text, path/profile/network indicators, and excerpt.
...
4. Keep ambiguous or one-off events in the detector's unmapped rule, such as `ps-unmapped-001`.
...
Use separate percentages for separate questions:

- `shell_call_efficiency_percent`: `(successful_shell_calls_seen / shell_calls_seen) * 100`.
- `detected_event_efficiency_percent`: `((shell_calls_seen - event_count) / shell_calls_seen) * 100`.
- `lost_time_percent`: `(unique shell output indexes inside detected-event repair windows / shell_calls_seen) * 100`.
- `efficiency_percent`: `100 - lost_time_percent`.
- `repair_turn_burden_percent`: `(sum(rework_turns_to_resolve) / shell_calls_seen) * 100`; this can exceed the unique lost-time estimate because repair windows can overlap.
- `summary.efficiency_by_error_family`: same lost-time estimate grouped by `error_family`; family repair windows can overlap, so family shares may sum above 100%.

Treat `lost_time_percent` as repair-turn burden, not wall-clock time. It depends on repair detection confidence and should be interpreted with `repair_detection_mode` and `repair_confidence_label`.
