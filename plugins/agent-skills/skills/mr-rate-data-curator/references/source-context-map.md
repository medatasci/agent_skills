...
| Local `tools/build_mr_rate_db.py` | SQLite import behavior and table creation. | Require workspace tool check and database-write approval. |
| `scripts/curate_mr_rate_data.py` | Restartable wrapper over download, import, run, and status commands. | Provide deterministic local command surface. |
...
| MR-RATE SQLite Query Analyst | Defer. | Useful later, but should be separate from download/import side effects. |
...
inspection without writing data.
