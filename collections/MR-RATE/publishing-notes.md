...
The MR-RATE collection is packaged as a publication wrapper around 14
...
review is complete in `publication-review-matrix.md`. The final aggregate
catalog rebuild excludes the local unreviewed `skills/mr-rate-data-curator/`
folder from generated package surfaces. The next publication gate is maintainer
review, then optional git staging, commit, and remote pull request creation.
...
- `skills/mr-rate-data-curator/` is not part of this package unless it receives
  a separate review and evaluation pass.
...
- [x] Search the 14 skill folders for local absolute paths.
- [x] Search the 14 skill folders for credentials, tokens, keys, or private
      identifiers.
...
- [x] Confirm `catalog/skills/<skill-id>.json` exists for all 14.
- [x] Confirm `catalog/skills.json` includes all 14.
- [x] Confirm `catalog/search-index.json` includes all 14.
- [x] Confirm plugin mirrors exist for all 14.
- [x] Confirm static site pages exist for all 14.
- [x] Confirm `site/.well-known/agent-skills/index.json` includes all 14.

### 7. Evaluation

- [x] Run `python -m skillforge evaluate <skill-id> --json` for all 14.
- [x] Confirm all pass.
...
- [x] Include all 14 `skills/mrrate-*` source folders.
